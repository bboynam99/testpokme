...
io.sockets.on("connection", function(socket) {
  ...
  socket.on("ducnghialogin", function(data, callback) {
    ...
    if (players.id.indexOf(ducnghia_data[1]) != -1) {
      console.log("" + ducnghia_data[1] + " Da ket noi toi tro choi roi,");
    } else {
  ...
  socket.on("Move", function(data) {
    ...
    players.info[i].buff = ducnghia_data[18];
    ...
    //io.sockets.emit("online", 'ducnghia'+players.id.length);

    for (var i2 in pokemon) {
      if (pokemon[i2].hp <= 0 && pokemon[i2].time <= Date.now()) {
        pokemon[i2].hp = pokemon[i2].hpfull;
        pokemon[i2].time = 0;
        if (+pokemon[i2].hoisinh >= 1800000) {
          io.sockets.emit("chatthegioi", {
            msg: " BOSS " + pokemon[i2].name + " đã xuất hiện. ",
            id: 0,
            type: "CTG"
          });
        }
    ...
    if (pokemon.length <= 0) {
      io.sockets.emit("loadquai", "1");
    }

    var boxquai = [];
    for (var i in pokemon) {
      if (+pokemon[i].map == +ducnghia_data[2] && pokemon[i].hp >= 1) {
        boxquai.push(pokemon[i]);
      }
    }
    io.sockets.emit("laypokemon", boxquai);

    for (var k in vatpham) {
      if (vatpham[k].timemat >= Date.now()) {
        io.sockets.emit("itemtomap", {
          id: vatpham[k].id,
          map: vatpham[k].map,
          x: vatpham[k].x,
          y: vatpham[k].y,
      ...
      if (vatpham[k].id == data.id) {
        if (vatpham[k].timenhat >= Date.now()) {
          if (vatpham[k].uid == data.uid) {
            io.sockets.emit("donenhat", {
              idvp: vatpham[k].idvp,
              soluong: vatpham[k].soluong,
              uid: data.uid,
              code: 1
            });
            io.sockets.emit("xoavp", { id: vatpham[k].id });
  ...
  socket.on("pokeball", function(data) {
    for (var i in pokemon) {
      if (pokemon[i].id == data.idpkm) {
        io.to(socket.id).emit("pokeball", {
          info: pokemon[i],
          uid: data.uid,
          idvp: data.idvp
        });
  ...
  socket.on("gietnguoi", function(data) {
    io.sockets.emit("gietnguoi", data);
  });
  //pvp_moi
  ...
  socket.on("ngonngu", function(data) {
    io.sockets.emit("ngonngu", data);
  });
  socket.on("buff", function(data) {
    io.sockets.emit("buff", data);
  });

  socket.on("dichchuyen", function(data) {
    io.sockets.emit("dichchuyen", { id: data.id, uid: data.uid });
  });

  ///pvp game ////

  socket.on("ban_1", function(data) {
    io.sockets.emit("ban_1", data);
  });

  socket.on("pokemon_1", function(data) {
    io.sockets.emit("pokemon_1", data);
  });

  socket.on("ban_2", function(data) {
    io.sockets.emit("ban_2", data);
  });

  socket.on("pokemon_2", function(data) {
    io.sockets.emit("pokemon_2", data);
  });

  socket.on("ban_3", function(data) {
    io.sockets.emit("ban_3", data);
  });

  socket.on("pokemon_3", function(data) {
    io.sockets.emit("pokemon_3", data);
  });
  ...
  socket.on("dame", function(data) {
    for (var i in pokemon) {
      if (pokemon[i].id == data.id) {
        ...
        if (pokemon[i].hp <= data.dame && pokemon[i].hp >=1) {
          pokemon[i].hp = 0;
          ...
          io.to(socket.id).emit("laykiss", {
            info: pokemon[i],
            dame: data.dame
          });
        }
    io.sockets.emit("truhp", {id : data.id , hp :  pokemon[i].hp});
